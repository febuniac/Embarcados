...
//include das bibliotecas do Driver
...
int main(void)
{
	...
	while(1){
		...
		* 0 : apertado
		*/
	    if(BUT_PIO->PIO_PDSR & (BUT_PIN_MASK)){//_pio_get_output_data_status
			_pio_clear(LED_PIO,LED_PIN_MASK);//LED_PIO->PIO_CODR = LED_PIN_MASK;
        }
		else{
			_pio_set(LED_PIO,LED_PIN_MASK);////LED_PIO->PIO_SODR = LED_PIN_MASK;
        }
	};
}
